...
public class ProxyFactoryTest {
	...
	private final ProxyFactory aufProxyFactory = new ProxyFactory(correlMap, mqSink);
	...
	public void general001() {
		Assertions.assertTrue(aufProxyFactory.newInstance(GeneralTestCase.Case001.class) instanceof GeneralTestCase.Case001);
	}

	@Test
	public void general002() {
		Assertions.assertEquals(2, aufProxyFactory.newInstance(GeneralTestCase.Case001.class).inc(1), "Should call default implementation");
	...
	public void destination001() {
		final var newInstance = new ProxyFactory(correlMap, mqSink).newInstance(DestinationTestCase.Case001.class);
		
		newInstance.m001();
		
		Assertions.assertEquals("test.inbox", ref.get().getTo(), "Should be simple value of annotation");
	}
	
	@Test
	public void destination002() {
		final var newInstance = new ProxyFactory(correlMap, mqSink).newInstance(DestinationTestCase.Case002.class);
		
		newInstance.m001();
		
		Assertions.assertEquals("", ref.get().getTo(), "Should not happen since the interface won't be scanned and proxied");
	}
	/**
	 * ReplyTo
	 */
	...
	public void correlationId001() {
		final var newInstance = new ProxyFactory(correlMap, mqSink).newInstance(TypeTestCase.TypeCase001.class);
	...
	@Test
	public void body001() {
		final var newInstance = new ProxyFactory(correlMap, mqSink).newInstance(BodyTestCase.class);
		newInstance.m001();
