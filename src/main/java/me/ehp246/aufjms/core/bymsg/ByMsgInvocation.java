...
class ByMsgInvocation implements MsgSupplier, ResolvedExecutable {
    ...
    static {
        try {
            ONREPLY = ByMsgInvocation.class.getDeclaredMethod("onReply", Received.class);
    ...
    public void onReply(final Received msg) {
        LOGGER.trace("Received reply");

        try {
            /*
             * if (msg.isException()) { this.fromBody.from(msg.getBodyAsText(), new
             * FromBody.Receiver<ExecutionThrown>() {
             * 
             * @Override public Class<ExecutionThrown> getType() { return
             * ExecutionThrown.class; }
             * 
             * @Override public void receive(final ExecutionThrown value) {
             * future.completeExceptionally(new ForMsgExecutionException(value.getCode(),
             * value.getMessage())); } }); return; }
             */

            /*
             * this.fromBody.from(msg.getBodyAsText(), new FromBody.Receiver<>() {
             * 
             * @Override public List<? extends Annotation> getAnnotations() { return
             * List.of(invoked.getMethod().getAnnotations()); }
             * 
             * @Override public Class<?> getType() { return invoked.getReturnType(); }
             * 
             * @Override public void receive(final Object value) { future.complete(value); }
             * 
             * });
             */
        } catch (final Exception e) {
